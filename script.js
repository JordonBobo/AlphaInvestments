...
// My stuff to add =============================================================
  ...
  for (let i = 0; i < 52; i++) {
    ...
    closingValue1.unshift(response["Weekly Time Series"][priceDate]["4. close"]);
  ...
  visualData()
    ...
    var high = "$" + testArray.reduce(function(a,b) {
      return Math.max(a,b);
    });
    var low = "$" + testArray.reduce(function(a,b) {
      return Math.min(a,b);
    });
    ...
    var changeStock = percentageChange(testCurrentDay,testPreviousDay);
      if (changeStock > 0) {$("#percent").attr("class", "green")}
      else if (changeStock <0) {$("#percent").attr("class", "red")}
      else {$("#percent").attr("class", "black")}
    console.log();
